...
public class mts extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session != null && session.getAttribute("username") != null && session.getAttribute("username") != null) {
            ...
            try {               
                r = SQLConnection.GetFiveColTwoPara("procGetManagerTimesheets", "Cnt", "TimeSheetID", "Period", "Status", "Comment", username, typ);
                
                while (r.next()) {
                    html = html + " <tr><td>" + r.getString("Cnt") + "</td><td><a href=" + rooturl + "mnts?id=" + r.getString("TimeSheetID") + ">" + r.getString("Period") + "</a></td><td>" + r.getString("Status") + "</td><td>" + r.getString("Comment") + "</td></tr>";
            ...
            request.setAttribute("url", rooturl);
            request.getRequestDispatcher("/WEB-INF/views/mts.jsp").forward(request, response);
        } else {
            String rooturl = request.getRequestURL().toString().substring(0, request.getRequestURL().toString().indexOf("com.EmployeeTimeCapsule.TimeCapsule")) + "com.EmployeeTimeCapsule.TimeCapsule/";
            request.setAttribute("url", rooturl);
